...
export default function Page() {
  ...
  if (session?.user) {
    if(session?.user.statusId != 2){
      return (<><main className={styles.main}><p>Your account is not approved</p></main></>)
    }

    if(session?.user.userBlock){
      return (<><main className={styles.main}><p>Your account is blocked</p>
      <p>conatct admin at help@fundmeapp.com</p></main></>)
    }
