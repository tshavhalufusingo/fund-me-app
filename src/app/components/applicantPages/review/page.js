...
export default function Review() {
    ...
    useEffect(() => {
        const fetchApplications = async () => { // Function to fetch applications from the API
            try {
    ...
    return (
        <main className={styles.main}> {/* Main content container with styling */}
            ...
            <ul>
                {applications.map(application => ( // Map through applications and render each application
                    application.userId == userID ? ( // Check if the application belongs to the current user
                        <li key={application.applicationId}> {/* List item for each application */}
                            <div className='applicantsreview'> {/* Container for application details */}
                                <p>Application title: {application.companyName}</p> {/* Company name */}
                                <p>Application date: {application.applicationDate}</p> {/* Application date */}
                                <p>Status: {getStatusText(application.statusId)}</p> {/* Application status */}
                            </div>
                        </li>
