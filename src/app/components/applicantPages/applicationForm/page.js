...
export default function ApplicationForm() {
  ...
  const handleFile = (event) => {
    ...
    fileReader.onloadend = () => {
      ...
      setDocuments((prev) => [
        ...prev,
        { type: documentType, content: base64data, name: file.name },
      ]); // Update the documents state with the new file
    };
  };
  ...
  const handleApplication = async (e) => {
    ...
    try {
      ...
      for (const document of documents) {
        // Iterate through uploaded documents
        const pdf = {
          // Create PDF object for attachment
        ...
        await fetch("/api/attachments", {
          // Send POST request to save attachment
          method: "POST", // Use POST method
          headers: {
            "Content-Type": "application/json", // Set content type header to JSON
          },
  ...
  return (
    <main className={styles.main}>
      ...
      <form className="applicationForm" onSubmit={handleApplication}>
        {" "}
        {/* Application form with submit handler */}
        ...
        {/* Label for document type selection */}
        <select id="documentType" name="documentType" required>
          {" "}
          {/* Dropdown for selecting document type */}
          <option value="Identity Document">ID</option>{" "}
          {/* Option for identity document */}
          <option value="CV">CV</option> {/* Option for CV */}
          <option value="Motivation">Motivation</option>{" "}
          {/* Option for motivation letter */}
          <option value="Other">Other</option>{" "}
          {/* Option for other documents */}
        </select>
        <div className="documentsDiv" id="documentsDiv">
          {" "}
          {/* Container for displaying uploaded documents */}
          {documents.map(
            (
              doc,
              index // Map through uploaded documents and display them
            ) => (
              <div key={index}>
                {" "}
                {/* Display document name and type */}
