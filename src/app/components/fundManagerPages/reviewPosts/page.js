...
export default function ReviewP() {
    ...
    const generateAdditionalData = (application) => {
        return {
            additionalKey: `extraValueFor-${application.applicationId[0]}`,
            anotherKey: `anotherValueBasedOn-${application.firstname}`
        };
    };
    ...
    const checkingMultipleUpload = (myjson) => {
      ...
      myjson.forEach((post) => {
        ...
        if (postMap.has(postId)) {
          ...
          // Only update if the attachment is new and not null
          if (attachment && !existingPost.arrayAttachments.includes(attachment)) {
            existingPost.arrayAttachments.push(attachment);
          }
        } else {
          postMap.set(postId, {
            ...post,
            arrayAttachments: attachment ? [attachment] : [], 
          });
        }
      });
        ...
        const fetchData = async () => {
            try {
                if (userId) {
                    ...
                    // Combine each application with dynamically generated additional data
                    const combinedData = data.map(application => ({
                        ...application,
                        ...generateAdditionalData(application)
                    }));

                    console.log("Combined Data:", combinedData);

                    const processedData = checkingMultipleUpload(combinedData);
